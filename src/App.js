...
const Player = ({ position, onMove, Success, onSuccessComplete }) => {
  ...
  useEffect(() => {
    ...
    if (Success) {
      ...
      // Set timeout to reset success state
      successTimeoutRef.current = setTimeout(() => {
        if (successInput && !successInput.isDisposed) {
          successInput.value = 0;
          onSuccessComplete();
        }
      }, 500); // Match SUCCESS_ANIMATION_DURATION
    }
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      ...
      if (keys.has('ArrowLeft')) deltaX -= MOVE_SPEED;
      if (keys.has('ArrowRight')) deltaX += MOVE_SPEED;
      if (keys.has('ArrowUp')) deltaY -= MOVE_SPEED;
      if (keys.has('ArrowDown')) deltaY += MOVE_SPEED;
...
export default function App() {
  ...
  return (
    <div className="game-container">
      ...
      {grasses.map(grass => (
        <GrassTarget key={grass.id} position={grass.position} />
      ))}
